...
export default async function Page() {
    ...
    const { data, error } = await supabase
        ...
        .limit(3);
    const { data: attandance, error: attandanceerror } = await supabase.schema("sekolah").from("attendance").select("*").eq("student_id", student.id)
    ...
    return (
        <main className={`${jakarta.className} flex flex-col items-center pb-8`} >
            ...
            <div className={"flex flex-col mt-6 h-max items-center w-full gap-3 px-4 " + inter.className} >
                ...
                {data.map(e =>
                    <Assignmentlist status={completed} late={false} key={e.id} id={e.id} tanggal={e.deadline} tugas={e.name} description={e.description} namaguru={e.teacher_id.name} mapel={"b ind"} />
                )}
            </div>
