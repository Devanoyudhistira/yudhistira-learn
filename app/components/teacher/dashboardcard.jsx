...
export default function Dashboardcard({studentid,studentname,assignmentlist}) {
    return (
        <div className="flex w-full px-3 shadow-md shadow-black/20 rounded-2xl py-3 flex-col gap-2" >
            ...
            <div className="flex gap-3 pb-2 items-center flex-nowrap overflow-x-scroll" >
                {assignmentlist.map(e => <Assignmentrating assignmentname={e.name} assignmentid={e.id} studentid={studentid} key={e.id} />      )}                           
            </div>
        </div>
    )
}